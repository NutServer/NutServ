...
bool Request::crlfCheck(std::vector<char> &raw_data, bool two){

	const char *crlf[] = {"\r\n", "\r\n\r\n"};
	std::vector<char>::iterator it_crlf;
	std::vector<char>::iterator searchStart = raw_data.begin() + readPos;

	switch (two){
		case true:
			it_crlf = std::search(searchStart, raw_data.end(),  crlf[1], crlf[1] + 4);
			if (it_crlf != raw_data.end())
				return true;
			break ;
		case false:
			it_crlf = std::search(searchStart, raw_data.end(),  crlf[0], crlf[0] + 2);
			if (it_crlf != raw_data.end()){
				readPos = std::distance(raw_data.begin(), it_crlf) + 2;
				return true;
			}
			break ;
	}
...
void Request::parseRequestLine(){
	...
	_Request = std::string(raw_data.begin() + _lastPos, raw_data.begin() + (readPos - 2));
	std::stringstream ss(_Request);

	ss >> _method >> _path >> _version;
	if (ss.fail()) { // cehck if the last extraction failed (eg, the request was GET / or GET /upload )
		throw Request::BadRequestException404();
	}
	// check version + methode
	if ((_version != "HTTP/1.1")
		|| !methods.count(_method)) {
...
void Request::parseHeaders(){
	...
	while(crlfCheck(raw_data, false)){
		std::string line = std::string(raw_data.begin() + _lastPos, raw_data.begin() + (readPos - 2));
		if(line.empty()){
		...
		switch (_currentState) {
			...
			case READ_BODY:
				if (raw_data.size() - readPos >= _expectedSize) {
					...
					_body.assign(raw_data.begin() + readPos, raw_data.begin() + readPos + _expectedSize);
			...
			case READ_CHUNKED:
				if(chunkEndCheck){
					...
					if(crlfCheck(raw_data, false)){
						std::string sizeline = std::string(raw_data.begin() + _lastPos, raw_data.begin() + (readPos - 2));
						std::stringstream ss;
						ss << std::hex << sizeline;
						if((ss >> _chunkSize)){
							_lastPos = readPos;
						...
						if(raw_data.size() - readPos >= _chunkSize + 2){
							_body.insert(_body.end(), raw_data.begin() + _lastPos, raw_data.begin() + _lastPos + _chunkSize);
